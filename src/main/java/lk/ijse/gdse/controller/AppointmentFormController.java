...
import lk.ijse.gdse.dto.tm.AppointmentCartTM;
import lk.ijse.gdse.dto.tm.AppointmentTM;
        ...
        if (tblCart.getItems().size() != 0) {
            if (!(lblAppointmentId.getText().isEmpty() || appointtmentDate.getValue() == null || appointmentTime.getValue() == null || cmbCustomer.getSelectionModel().getSelectedItem() == null)) {
                if (!appointtmentDate.getValue().isBefore(LocalDate.now())) {
                    ...
                    List<AppointmentInventory> inventoryList = new ArrayList<>();
                    ...
                    for (int i = 0; i < tblCart.getItems().size(); i++) {
                        ...
                        try {
                            ...
                            inventoryList.add(new AppointmentInventory(appointmentId, inventoryId, cartTM.getQty()));
                        } catch (SQLException e) {
                            e.printStackTrace();
                            new Alert(Alert.AlertType.ERROR, "Something went wrong!").show();
                        }
